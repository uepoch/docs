...
This will generate a `d.ts` file and (by default) save it to `.wrangler/types/runtime.d.ts`. It will also generate a `worker-configuration.d.ts` in your top-level directory. You will want to add these files to your `tsconfig.json`'s `compilerOptions.types` array.
